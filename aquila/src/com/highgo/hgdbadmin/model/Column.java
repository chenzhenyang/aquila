...
public class Column {
	...
	public String toSql() {
		...
		//默认值先不处理
		/**
		if (difault != null && (!"".equals(difault))) {
			if (DataTypeMap.isNumberic(dataType)) {
				sb.append(" DEFAULT " + difault);
			} else {
				// [TestSequence]
				// Sequence 最后加上，不要在这加，这样加名字是随机的
				// if (difault.startsWith("(NEXT VALUE FOR")) {
				// int start = difault.indexOf('[');
				// int end = difault.indexOf(']');
				// String sequenceName = difault.substring(start, end);
